...
router.get('/:user_id', isLoggedIn, async (req, res, next) => {
  try {
    ...
    if (err.name === 'TypeError') return res.status(203).json({success: false, message: '없는 유저의 인덱스 입니다.'})
    console.error(err)
    next(err)
  }
})
...
router.get('/comments/:reviewed_user_id', isLoggedIn, async (req, res, next) => {
  try {
    ...
    if (scoreAndComments.length === 0) return res.status(203).json({success: false, message: '유저에 해당하는 코멘트가 없습니다.'})
...
router.put('/rating/:reviewed_index/:user_review_index', isLoggedIn, async (req, res, next) => {
  try {
    ...
    // update한 부분이 없는 경우
    if (result[0] === 0) return res.status(203).json({success: false, message: '해당하는 평가혹은 평가에 변화가 없습니다.'})
...
router.delete('/rating/:reviewed_index/:user_review_index', isLoggedIn, async (req, res, next) => {
  try {
    ...
    // 전달받은 커멘트가 없는경우
    if (result === 0) return res.status(203).json({success: false, message: '해당하는 평가가 없습니다.'})
